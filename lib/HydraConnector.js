...
var HydraError = require('./HydraError.js').HydraError;
...
/**
 * A reference to the HydraError prototype.
 */
HydraConnector.HydraError = HydraError;
...
function composeOperationCallback(operation, callback) {
    return function(data, response) {
        if(response.statusCode >= 200 && response.statusCode < 300) {
            ...
            callback(new HydraError(response.statusCode, operation + " operation returned status: " + response.statusCode));
        }
    }
}
...
HydraConnector.prototype.login = function(username, password, callback) {
    ...
    self.client.post(self.url + "/login", {
        ...
        if(response.statusCode == 200) {
            ...
            callback(new HydraError(response.statusCode, data.error));
        }
    });
};
...
HydraConnector.prototype.logout = function(callback) {
    ...
    self.client.post(self.url + "/logout", {
        ...
        if(response.statusCode == 204) {
            ...
            callback(new HydraError(response.statusCode, "Logout returned status: "+response.statusCode));
        }
    });
};
...
HydraConnector.prototype.downloadFile = function(path, ws, callback) {
    ...
    httpModule.get({
        ...
        if(response.statusCode == 302) {
            ...
            callback(new HydraError(response.statusCode, "Download file operation returned status: "+response.statusCode));
        } else {
            response.pipe(ws);
...
/**
 * Queries the properties of a build. The buildstatus field can be interpreted
 * by invoking the {@link HydraConnector.determineBuildStatus} function.
 *
 * @method
...
/**
 * Determines the meaning of a build status integer value.
 *
 * @method
 * @param {Number} buildstatus Build status integer value
 * @return {String} A string representation of the build status
 */
HydraConnector.determineBuildStatus = function(buildstatus) {
    if(buildstatus === null) {
        return "";
    } else {
        switch(buildstatus) {
            ...
            case 7:
                return "Timed out";
            case 9:
                return "Unsupported system type";
            case 10:
